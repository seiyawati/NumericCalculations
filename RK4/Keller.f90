program Runge_Kutta_method
  ...
  function bibun2 (r,v)
    implicit none
    double precision, intent(IN) :: r, v
    double precision :: bibun2, Pb, Pg, Ps, k3, a, b, c, d
    ...
    a = 1.5d0 * (1 - v/(3*Va)) * (v**2)

    b = (1 + v/Va)/Rho_L * (Pb - Pinf)

    c = r/(Va*Rho_L) * ((-k3 * (v/r) * Pg) + (2.0d0 * sigma * v * (r**(-2))))

    d = (1 - v/Va) / r

    bibun2 = (b + c - a) * d

    ! bibun2 = ( (Pb  - Pinf) / Rho_L - 1.5d0 * (v**2) ) / r
